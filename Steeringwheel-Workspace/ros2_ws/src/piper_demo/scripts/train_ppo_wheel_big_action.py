...
class WheelHoldIsaacRosEnv(gym.Env):
    def __init__(self):
        ...
        self.observation_space = gym.spaces.Box(
            low=np.array([-180.0, -500.0, -self.max_action_deg], dtype=np.float32),
            high=np.array([180.0, 500.0, self.max_action_deg], dtype=np.float32),
            dtype=np.float32,
        )
        ...
        print("BIG ACTION PPO training started")
        print("Input:  /wheel/position")
        print("Output: /ai/wheel_hold_action")
        print(f"Action range: -{self.max_action_deg} to +{self.max_action_deg} deg")
    ...
    def reset(self, seed=None, options=None):
        ...
        print("")
        print("======================================")
        print(f"New episode target: {self.target_angle:.3f} deg")
        print("Goal: hold current wheel angle")
        print("======================================")
        print("")
    ...
    def step(self, action):
        ...
        reward = -0.1 * (error ** 2) - 0.08 * abs(action_deg)
